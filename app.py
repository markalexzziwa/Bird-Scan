            ...
            if st.button("Identify Specie", key="identify_specie_upload_button"):
                if model is not None and label_map is not None:
                    ...
                    if result:
                        ...
                        st.error("Failed to predict species.")
                else:
                    st.error("Model or label map not loaded.")

        # ────────────────────────────────────────────────────────
        # 1. Show result + "See more" button
        # ────────────────────────────────────────────────────────
            ...
            # "See more" button
                if st.button("See more", type="secondary", key="see_more_btn"):
                    st.session_state.show_video_generator = True
                    st.session_state.predicted_bird = result['species'].strip().title()

        # ────────────────────────────────────────────────────────
        # 2. Uganda Bird Video Generator (only shows after "See more")
        # ────────────────────────────────────────────────────────
            if st.session_state.get("show_video_generator", False):
                predicted_name = st.session_state.predicted_bird

                st.markdown("---")
                st.title("Uganda Bird Video Generator")
                st.caption(f"Auto-filled with: **{predicted_name}**")
            ...
            # Pre-filled search bar
                bird_name = st.text_input(
                    ...
                    key="auto_filled_bird",
                    placeholder="e.g. African Jacana"
                ).strip().title()

            # Auto-generate video if name is valid
                if bird_name in bird_db:
                    ...
                    st.error(f"**{bird_name}** not in database.")

        # Optional: Reset button
            if st.session_state.get("show_video_generator", False):
                if st.button("Start Over"):
                    for key in ["upload_result", "show_video_generator", "predicted_bird"]:
